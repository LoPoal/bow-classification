# -*- coding: utf-8 -*-
"""
Created on Tue Mar 11 16:31:30 2014

@author: master
"""
from numpy.random import randint
from numpy import array, arange, compress, equal, take, mean
from scipy.cluster.vq import vq   # for k-means
import numpy as np
import multiprocessing as mp

def mcv_kmeans(obs, k_or_guess, iter=20, thresh=1e-5, multiprocess=0):
    """
    ...
    if type(k_or_guess) == type(array([])):
        ...
        if multiprocess:
            pool = mp.Pool(mp.cpu_count()+ (multiprocess+1) if mp.cpu_count()>1 else 0)
            PoolResults = []

        for i in range(iter):
            #the intial code book is randomly selected from observations
            guess = take(obs, randint(0, No, k), 0)
            if multiprocess:         ########### start change
                PoolResults.append(pool.apply_async(_kmeans, args=(obs, guess, thresh)))
            else:
        ...
        if multiprocess:
            ...
            while (len(PoolResults)>0):
                try:
                    book, dist = PoolResults[0].get(timeout=0.001)
                    PoolResults.pop(0)
                    if dist < best_dist:
                        best_book = book
                        best_dist = dist
